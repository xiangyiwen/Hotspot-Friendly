...
long_txn_ratio=(0.0001 0.001 0.01 0.05 0.1)
...
sed -i '161s/.*/#define LONG_TXN_READ_RATIO			1/g' config.h
sed -i '164s/.*/#define SYNTHETIC_YCSB              true/g' config.h
sed -i '165s/.*/#define POS_HS                      TOP/g' config.h
...
do
  ...
  for((i=0;i<${#long_txn_ratio[@]};i++))
  do
    sed -i '160s/.*/#define LONG_TXN_RATIO              '${long_txn_ratio[$i]}'/g' config.h
    make -j
    ./rundb
  done
done
